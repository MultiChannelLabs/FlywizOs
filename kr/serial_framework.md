...
# 통신 프레임워크 설명
 ...
 이 장에서는 통신 프레임 워크의 구현 원리에 초점을 맞추고 있고, 이론적인 것들이 비교적 많습니다. 처음에는 통신 모델에 대한 대략적인 개요대해 이해하고, 제공되는 [Case](serial_example.md)를 직접 수행해 본 후 다시 돌아와 원리를 이해해 보겠습니다. 이러한 과정을 거치고 실제 수행을 하다보면 원하는 방식으로 프로토콜을 정의하고 사용할 수 있을 것입니다.
...
* UART - 프로토콜 분석 및 캡슐화의 시리얼 포트 HAL 계층
	* UartContext：시리얼 포트 스위치, 송수신 인터페이스를 제공하는 시리얼 포트의 물리적 제어 계층
	* ProtocolData：통신 프로토콜에서 변환 된 실제 변수를 저장하는 데 사용되는 통신 데이터 구조 정의
	* ProtocolSender：데이터 전송 캡슐화 완성
	* ProtocolParser：데이터의 프로토콜 분석을 완료 한 다음 구문 분석 된 데이터를 ProtocolData의 데이터 구조체에 넣는 동시에 어플리케이션이 시리얼 데이터 변경을 모니터링 할 수 있도록 콜백 인터페이스를 관리합니다.
* APP - 어플리케이션 인터페이스 계층
...
**receive** 및 **send** 두 프로세스가 위아래로 진행되고 각 레이어의 기능이 비교적 명확하다는 것을 분명히 알 수 있습니다.

코드에 해당하는 구체적인 과정은 다음과 같습니다.

![](images/serial_model_detail2.png)

 수신 또는 송신 프로세스에 관계없이 시리얼 포트에 대한 최종 읽기 및 쓰기 작업은 'UartContext'를 통해 이루어집니다. 이것은 표준화 된 프로세스이므로 기본적으로 `UartContext`를 수정할 필요가 없으며 구현 방법을 무시할 수 있습니다. 예, 물론 관심이 있으시면 확인하실 수 있습니다.
...
ProtocolParser.cpp 파일, 구성 파일 명령 형식
```c++
/**
...
```
 위의 분석 과정은 약간 복잡합니다. 먼저 아래 그림을 본 후 분석하면 이해하기 더 쉬울 것입니다. 데이터 패킷은 0 또는 여러 프레임의 데이터를 포함 할 수 있습니다. 아래 그림에서는 3개의 프레임 데이터를 표시했습니다. 그리고 아직 데이터가 5byte부족한 불완전한 데이터 프레임이 있습니다. 불완전한 데이터 프레임은 다음 데이터 패킷에 이어지게됩니다.
...
### 통신 프로토콜 데이터를 UI 컨트롤과 연결하는 방법
이전 프로토콜 프레임 워크를 계속하여 procParse의 구문 분석 부분에 들어갑니다.
...
```
 ...
 데이터 분석이 완료되면 `notifyProtocolDataUpdate`를 통해 UI 업데이트를 알립니다. 이 부분은 아래 UI 업데이트 부분을 참고하시기 바랍니다.
...
```
코드에는 페이지의 정적 변수 인 mProtocolData 변수가 있습니다. onUI_init () 중에 초기화됩니다.
예 :
...
## 시리얼 데이터 전송
 ProtocolSender.cpp를 엽니 다.
 APP 계층이 MCU (또는 기타 장치)에 데이터를 보내야하는 경우 sendProtocol을 직접 호출하는 것으로 충분합니다.
 특정 프로토콜 캡슐화는 sendProtocol 함수에 의해 완료됩니다. 사용자는 자신의 프로토콜 요구 사항에 따라 이 부분의 코드를 수정할 수 있습니다.
