...
たとえば、**main.ftu** UIファイルに2つのボタンコントロールを追加し、各IDを`Button1`と`Button2`で作らコンパイルすると、次の2つの機能が** mainLogic.cc**ファイルの作成されます。
...
* IDEは、コンパイル時のコントロールのIDに基づいて関連する関数を作成するのには、もし同じ関数が既に存在する場合、そのプロセスをスキップし、** Logic.cc**ファイルは変更されません。
